#modified from Burns et al. Contribution of neutral processes to the assembly of the gut microbial communities changes over host development
...
sncm.fit <- function(spp, pool=NULL, taxon=NULL){
	require(minpack.lm)
	require(Hmisc)
	require(stats4)
	...
	m.mle <- try(mle(sncm.LL, start=list(m=0.1, sigma=0.1), nobs=length(p), method="Nelder-Mead"))
	if (class(m.mle)=="try-error") {
		print("Trying alternative maximum likelihood estimation algorithm from bblme package")
		m.mle <- bbmle::mle2(sncm.LL, start=list(m=0.1, sigma=0.1), method="Nelder-Mead")
	}
	...
	bino.LL <- function(mu, sigma){
		R = 
		 - pbinom(d, floor(N), p, lower.tail=FALSE)
		R = dnorm(R, mu, sigma)
		-sum(log(R))
	}
	...
	return(list("stats"=fitstats, "vals"=B))
